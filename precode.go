...
func deleteTask(w http.ResponseWriter, r *http.Request) {
	...
	_, ok := tasks[id] // проверяю есть ли задача с таким ID
	if !ok {
		http.Error(w, "Задача не найдена", http.StatusBadRequest)
		return
	}

	delete(tasks, id)            // удаляю задачу с ID = 3
	w.WriteHeader(http.StatusOK) // устанавливаю статус успешного выполнения
}
